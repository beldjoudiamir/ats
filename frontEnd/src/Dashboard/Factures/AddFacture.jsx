...
const AddFacture = ({ onClose, onFactureAdded, mode = "add", initialData = null, onFactureEdited }) => {
  ...
  const [items, setItems] = useState([{ description: "", quantity: 1, price: 0 }]); // Articles dans la facture
  const [tvaRate, setTvaRate] = useState(20); // Taux de TVA
  const [factureList, setFactureList] = useState([]); // Liste des IDs de facture
  const [generatedID, setGeneratedID] = useState(""); // ID généré automatiquement pour la nouvelle facture
  ...
  useEffect(() => {
    if (mode === "edit" && initialData) {
      setGeneratedID(initialData.factureID || "");
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      const ids = await fetchAllFactureIDs();
      setFactureList(ids);
    };
    loadInitialData();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!companyInfo.length || !selectedClient) {
      console.error("Informations manquantes pour créer/modifier la facture.");
      return;
    }

    try {
      if (mode === "edit" && initialData) {
        await updateFacture(initialData._id, {
          factureID: generatedID,
        ...
        if (typeof onFactureEdited === "function") onFactureEdited();
        else if (typeof onClose === "function") onClose();
      } else {
        const ids = await fetchAllFactureIDs();
        const newID = generateFactureID(ids);
        ...
        if (typeof onFactureAdded === "function") onFactureAdded();
        else if (typeof onClose === "function") onClose();
      }
    } catch (error) {
  ...
  // Rendu du composant avec formulaire de création de facture
  return (
        ...
        <div className="p-8 bg-gray-100 min-h-screen flex justify-center items-center">
          <div className="bg-white rounded-lg w-full max-w-4xl p-6 border-1 border-gray-800">
            ...
            <h2 className="text-xl font-semibold mt-6 mb-6">
              {mode === "edit" ? (
                <>Modifier la facture N° : <span>{generatedID}</span></>
              ) : (
                <>Facture N° : {generatedID ? <span>{generatedID}</span> : <span>Génération en cours...</span>}</>
              )}
            </h2>

            {/* Formulaire de la facture */}
            <form onSubmit={handleSubmit}>
              {/* Champs Départ et Arrivée */}
              ...
              <button type="submit" className="mt-4 flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded w-full justify-center">
                <FaSave /> {mode === "edit" ? "Enregistrer les modifications" : "Enregistrer la facture"}
              </button>
            </form>
          </div>
        </div>
